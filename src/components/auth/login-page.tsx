...
export function LoginPage() {
  ...
  const submitSetup = async () => {
    ...
    try {
      ...
      settleSession(user)
      toast.success(t('welcomeAdmin', { name: name.trim() }))
      router.replace('/')
    } catch (e) {
      toast.error((e as Error).message)
      setPending(false)
  ...
  return (
    <div className="flex min-h-screen flex-col bg-muted/30">
      <div className="flex items-center justify-end gap-2 p-4">
        <LocaleSwitcher className="h-9 w-[130px]" />
        <ThemeToggle />
      </div>
        ...
        <div className="w-full max-w-sm">
          <div className="mb-6 flex flex-col items-center gap-3 text-center">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{tb('tagline')}</p>
            </div>
          </div>
          ...
          <div className="rounded-xl border bg-background p-6 shadow-sm">
            {mode === 'checking' ? (
              <div className="flex flex-col items-center gap-3 py-6 text-muted-foreground">
                ...
                <p className="text-sm">{ts('checking')}</p>
              </div>
            ) : isSetup ? (
              <>
                ...
                <p className="mb-4 text-xs leading-relaxed text-muted-foreground">
                  {t.rich('setupHint', { b: (chunks) => <b>{chunks}</b> })}
                </p>
                <form
                  onSubmit={(e) => {
                    e.preventDefault()
                ...
                >
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="login-password2">{t('passwordRepeat')}</Label>
                    <Input
                      id="login-password2"
                      type={showPassword ? 'text' : 'password'}
                      value={password2}
                  ...
                  <Button type="submit" className="w-full gap-1.5" disabled={pending}>
                    ...
                    {pending ? t('submitSetupPending') : t('submitSetup')}
                  </Button>
                </form>
              </>
            ) : (
              <>
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <h2 className="text-sm font-semibold">{t('login')}</h2>
                </div>
                <form
                  onSubmit={(e) => {
                    e.preventDefault()
          ...
          {mode === 'login' && (
            <p className="mt-4 text-center text-xs text-muted-foreground">{t('forgotHint')}</p>
          )}
        </div>
      </div>
    </div>
